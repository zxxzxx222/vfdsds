...
RUN echo 'zuu' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/zuu.sh

RUN echo 'cd ' >>/zuu.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/zuu.sh

RUN echo 'cd /noVNC-1.2.0' >>/zuu.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/zuu.sh

RUN chmod 755 /zuu.sh
